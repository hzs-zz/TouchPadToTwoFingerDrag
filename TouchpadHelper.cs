...
namespace TouchpadToMiddleClick
{
    public static class TouchpadHelper
    {
        ...
        public static Action? OnToggleHotkey;    // 对应 Alt+Shift+M (开关控制)
        public static Action? OnSettingsHotkey;  // 🌟 新增：对应 Alt+Shift+N (设置面板)
        ...
        public static bool Exists()
        {
            ...
            try
            {
                ...
                for (int i = 0; i < deviceCount; i++)
                {
                    ...
                    uint size = 0;
                    // 🌟 修复警告：接收并检查返回值
                    int sizeResult = GetRawInputDeviceInfo(ridl.hDevice, 0x2000000b, IntPtr.Zero, ref size);
                    ...
                    if (sizeResult == 0 && size > 0)
                    {
                        ...
                        {
                            // 🌟 修复警告：接收并检查返回值
                            int dataResult = GetRawInputDeviceInfo(ridl.hDevice, 0x2000000b, data, ref size);
                            if (dataResult >= 0)
                            {
        ...
        public static IntPtr RegisterInput()
        {
            ...
            // 注册快捷键 ID 1：Alt (0x0001) + Shift (0x0004) + M (0x4D)
            RegisterHotKey(hwnd, 1, 0x0001 | 0x0004, 0x4D);
            // 🌟 修复：补全丢失的快捷键 ID 2：Alt (0x0001) + Shift (0x0004) + N (0x4E)
            RegisterHotKey(hwnd, 2, 0x0001 | 0x0004, 0x4E);
        ...
        private static List<TouchpadContact>? GetTouchpadContacts(IntPtr lParam)
        {
            ...
            try
            {
                ...
                // 🌟 修复警告：检查预解析数据的大小
                int pSizeResult = GetRawInputDeviceInfo(rawInput.Header.hDevice, RIDI_PREPARSEDDATA, IntPtr.Zero, ref preparsedDataSize);
                if (pSizeResult != 0 || preparsedDataSize == 0) return null;
                ...
                {
                    // 🌟 修复警告：检查获取预解析数据的结果
                    int pDataResult = GetRawInputDeviceInfo(rawInput.Header.hDevice, RIDI_PREPARSEDDATA, preparsedDataPointer, ref preparsedDataSize);
                    if (pDataResult < 0 || pDataResult != preparsedDataSize) return null;
